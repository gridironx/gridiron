...
A user registers a native Cosmos Coin. Once the proposal passes (i.e is approved by governance), the ERC20 module uses a factory pattern to deploy an ERC20 token contract representation of the Cosmos Coin. Note that the native Gridiron coin cannot be registered, as any coin including "evm" in its denomination cannot be registered. Instead the Gridiron token can be converted by Nomand's wrapped Gridiron (WGRIDIRON) contract.

1. User submits a `RegisterCoinProposal`
2. Validators of the Gridiron Hub vote on the proposal using `MsgVote` and proposal passes
3. If Cosmos coin or IBC voucher exist on the bank module supply, create the [ERC20 token contract](https://github.com/gridiron/gridiron/blob/main/contracts/ERC20MinterBurnerDecimals.sol) on the EVM based on the ERC20Mintable ([ERC20Mintable by openzeppelin](https://github.com/OpenZeppelin/openzeppelin-contracts/tree/master/contracts/token/ERC20)) interface
    - Initial supply: 0
...
2. Validators of the GRIDIRON chain vote on the proposal using `MsgVote` and proposal passes
...
- There shouldn't exist any native Cosmos Coin ERC20 Contract (eg Gridiron, Atom,
