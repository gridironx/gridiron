	...
	Describe("Submitting a token pair proposal through governance", func() {
		Context("with existing coins", func() {
			BeforeEach(func() {
				...
				coins := sdk.NewCoins(
					sdk.NewCoin("agridiron", sdk.NewInt(1000000000000000000)),
			...
			Describe("for a single Cosmos Coin", func() {
				BeforeEach(func() {
					...
					_, err = testutil.Delegate(s.ctx, s.app, privKey, sdk.NewCoin("agridiron", sdk.NewInt(500000000000000000)), s.validator)
					s.Require().NoError(err)
...
func convertCoin(ctx sdk.Context, appGridiron *app.Gridiron, pk *ethsecp256k1.PrivKey, coin sdk.Coin) {
	...
	res, err := testutil.DeliverTx(ctx, appGridiron, pk, convertCoinMsg)
	s.Require().NoError(err)
...
func convertERC20(ctx sdk.Context, appGridiron *app.Gridiron, pk *ethsecp256k1.PrivKey, amt math.Int, contract common.Address) {
	...
	res, err := testutil.DeliverTx(ctx, appGridiron, pk, convertERC20Msg)
	s.Require().NoError(err)
