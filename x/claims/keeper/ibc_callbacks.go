...
func (k Keeper) OnRecvPacket(
	...
	// readable prefix (HRP) of the sender to `gridiron1`
	...
	// supported on Gridiron. To prevent funds getting stuck, return an error, unless
	...
	if sameAddress && !fromEVMChain {
		...
		case senderRecordFound && !senderClaimsRecord.HasClaimedAny():
			...
			return channeltypes.NewErrorAcknowledgement(
				errorsmod.Wrapf(
					gridiron.ErrKeyTypeNotSupported, "receiver address %s is not a valid ethereum address", recipientBech32,
				),
			)
		default:
