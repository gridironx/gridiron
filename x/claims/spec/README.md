...
This document specifies the internal `x/claims` module of the Gridiron Hub.

The `x/claims` module is part of the Gridiron [Rektdrop](https://gridiron.blog/the-gridiron-rektdrop-abbe931ba823) and aims to increase the distribution of the network tokens to a large number of users.
...
For the Gridiron Rektdrop, users are required to claim their airdrop by participating in core network activities. A Rektdrop recipient has to perform the following activities to get the allocated tokens:

* 25% is claimed by staking
* 25% is claimed by voting in governance
* 25% is claimed by using the EVM (deploy or interact with contract, transfer GRIDIRON through a web3 wallet)
