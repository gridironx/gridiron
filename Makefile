...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(GRIDIRON_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/gridiron:Z gridiron/node "./gridirond testnet init-files --v 4 -o /gridiron --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\gridirond:/gridiron\Z gridiron/node "./gridirond tendermint unsafe-reset-all --home=/gridiron"
	@docker run --rm -v $(CURDIR)\build\node1\gridirond:/gridiron\Z gridiron/node "./gridirond tendermint unsafe-reset-all --home=/gridiron"
	@docker run --rm -v $(CURDIR)\build\node2\gridirond:/gridiron\Z gridiron/node "./gridirond tendermint unsafe-reset-all --home=/gridiron"
	@docker run --rm -v $(CURDIR)\build\node3\gridirond:/gridiron\Z gridiron/node "./gridirond tendermint unsafe-reset-all --home=/gridiron"
else
	@docker run --rm -v $(CURDIR)/build/node0/gridirond:/gridiron:Z gridiron/node "./gridirond tendermint unsafe-reset-all --home=/gridiron"
	@docker run --rm -v $(CURDIR)/build/node1/gridirond:/gridiron:Z gridiron/node "./gridirond tendermint unsafe-reset-all --home=/gridiron"
	@docker run --rm -v $(CURDIR)/build/node2/gridirond:/gridiron:Z gridiron/node "./gridirond tendermint unsafe-reset-all --home=/gridiron"
	@docker run --rm -v $(CURDIR)/build/node3/gridirond:/gridiron:Z gridiron/node "./gridirond tendermint unsafe-reset-all --home=/gridiron"
endif
