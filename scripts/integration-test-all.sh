...
## default port prefixes for gridirond
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/gridirond keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/gridirond init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/gridirond add-genesis-account \
    "$("$PWD"/build/gridirond keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000agridiron,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/gridirond gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/gridirond collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/gridirond validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/gridirond start --pruning=nothing --rpc.unsafe \
    ...
    GRIDIRON_PID=$!
    echo "started gridiron node, pid=$GRIDIRON_PID"
    # add PID to array
    arr+=("$GRIDIRON_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test gridiron node $HOST_RPC ..."
