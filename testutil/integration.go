...
func SubmitProposal(
	...
	res, err := DeliverTx(ctx, appGridiron, pk, msg)
	if err != nil {
		return id, err
	}
...
func Delegate(
	...
	return DeliverTx(ctx, appGridiron, priv, delegateMsg)
}
...
func Vote(
	...
	return DeliverTx(ctx, appGridiron, priv, voteMsg)
}
...
func DeliverTx(
	...
	accountAddress := sdk.AccAddress(priv.PubKey().Address().Bytes())
	denom := appGridiron.ClaimsKeeper.GetParams(ctx).ClaimsDenom
	...
	seq, err := appGridiron.AccountKeeper.GetSequence(ctx, accountAddress)
	if err != nil {
		return abci.ResponseDeliverTx{}, err
	}
	...
	accNumber := appGridiron.AccountKeeper.GetAccount(ctx, accountAddress).GetAccountNumber()
	...
	res := appGridiron.BaseApp.DeliverTx(req)
	if res.Code != 0 {
