...
// NewRootCmd creates a new root command for gridirond. It is called once in the
// main function.
func NewRootCmd() (*cobra.Command, params.EncodingConfig) {
	...
	initClientCtx := client.Context{}.
		...
		WithKeyringOptions(gridironkr.Option()).
		WithViper(EnvPrefix).
...
func (a appCreator) appExport(
	...
	if height != -1 {
		gridironApp = app.NewGridiron(logger, db, traceStore, false, map[int64]bool{}, "", uint(1), a.encCfg, appOpts)

		if err := gridironApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		gridironApp = app.NewGridiron(logger, db, traceStore, true, map[int64]bool{}, "", uint(1), a.encCfg, appOpts)
	}

	return gridironApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs)
}
