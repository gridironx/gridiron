...
func (suite *UpgradeTestSuite) TestMigrateEscrowAcc() {
	suite.SetupTest(gridirontypes.MainnetChainID)
...
func (suite *UpgradeTestSuite) TestDistributeRewards() {
	// define constants
	mainnetChainID := gridirontypes.MainnetChainID + "-4"
	communityPool := sdk.MustAccAddressFromBech32("gridiron1jv65s3grqf6v6jl3dp4t6c9t9rk99cd8974jnh")
	...
	noRewardAddr := sdk.MustAccAddressFromBech32("gridiron1009egsf8sk3puq3aynt8eymmcqnneezkkvceav")
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				err := suite.app.BankKeeper.SendCoins(
					...
					sdk.NewCoins(
						sdk.NewCoin(gridirontypes.BaseDenom, balance.Quo(math.NewInt(2))),
					),
				)
				suite.NoError(err)
			},
		...
		{
			"Testnet - no-op",
			gridirontypes.TestnetChainID + "-4",
			func() {},
			false,
			math.ZeroInt(),
		},
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if gridirontypes.IsMainnet(tc.chainID) {
			...
			// balance should be 0
			balance := suite.app.BankKeeper.GetBalance(suite.ctx, noRewardAddr, gridirontypes.BaseDenom)
			...
			commPoolFinalBalance := suite.app.BankKeeper.GetBalance(suite.ctx, communityPool, gridirontypes.BaseDenom)
			...
			for i := range v11.Allocations {
				...
				balance := suite.app.BankKeeper.GetBalance(suite.ctx, addr, gridirontypes.BaseDenom)
				suite.Require().Equal(math.ZeroInt(), balance.Amount)
			...
			if tc.expectedSuccess {
				...
				finalFundingAccBalance := suite.app.BankKeeper.GetBalance(suite.ctx, fundingAcc, gridirontypes.BaseDenom)
...
func (suite *UpgradeTestSuite) fundTestnetRewardsAcc(amount math.Int) {
	...
	rewards := sdk.NewCoins(sdk.NewCoin(gridirontypes.BaseDenom, amount))
