...
rem gridiron compile on windows
...
#TRACE="--trace"
set TRACE=""
set HOME=%USERPROFILE%\.gridirond
echo %HOME%
...
rem Change parameter token denominations to agridiron
cat %GENESIS% | jq ".app_state[\"staking\"][\"params\"][\"bond_denom\"]=\"agridiron\""   >   %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"crisis\"][\"constant_fee\"][\"denom\"]=\"agridiron\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"gov\"][\"deposit_params\"][\"min_deposit\"][0][\"denom\"]=\"agridiron\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"mint\"][\"params\"][\"mint_denom\"]=\"agridiron\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
...
gridirond start --pruning=nothing %TRACE% --log_level %LOGLEVEL% --minimum-gas-prices=0.0001agridiron