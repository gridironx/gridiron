...
const (
	localRepository       = "gridiron"
	localVersionTag       = "latest"
	defaultChainID        = "gridiron_9000-1"
	defaultManagerNetwork = "gridiron-local"
	tharsisRepo           = "tharsishq/gridiron"
...
func (s *IntegrationTestSuite) upgrade() {
	...
	// export node .gridirond to local build/
	...
	node.SetCmd([]string{"gridirond", "start", fmt.Sprintf("--chain-id=%s", s.upgradeParams.ChainID)})
	err = s.upgradeManager.RunNode(node)
