...
Get started with your Ledger hardware wallet on Gridiron {synopsis}
...
If you want to connect to Gridiron mainnet and Gridiron testnet, you can use the Ethereum Ledger app on Ledger Live by setting the chain ID.
...
|               | EIP155 chain ID |
| ------------- | --------------- |
| Gridiron mainnet | `9001`          |
| Gridiron testnet | `9000`          |
...
::: tip
Follow our [Metamask Guide](./metamask.md) to add the Gridiron Mainnet and Testnet to your Settings
:::
...
To use your Ledger with the Gridiron CLI, first connect your device to your computer, unlock it using your PIN, and open the Ethereum app.

Then, connect your Ledger to the CLI with `keys add` command, and select a name for your device:

```
gridirond keys add NAME --ledger
```

**Example:**

```
gridirond keys add myledger --ledger

- address: gridiron1hnmrdr0jc2ve3ycxft0gcjjtrdkncpmmkeamf9
  name: myledger
...
```
gridirond tx bank send myledger gridiron1hnmrdr0jc2ve3ycxft0gcjjtrdkncpmmkeamf9 100000agridiron --fees 2000agridiron --from myledger
```
...
### Step-by Cosmos transaction using Gridiron.me

1. **Get your address in both encodings**

After connecting the Ledger wallet to Metamask and connecting to the [https://gridiron.me](https://gridiron.me) webpage, it will display our wallet formatted on `bech32` and `hex` representation, we need these values to make sure that the message that we are going to sign is the correct one.
...
- `msgs`: This is the content of the cosmos transaction, in this example, we need to make sure that we are using a MsgSend, and that the *to_address* is the one that we want to send the founds. Also, we can verify that we are actually sending *10000agridiron* to that wallet.
...
This step should be done automatically by the same service that generated the message, in this case, [gridiron.me](http://gridiron.me) will broadcast the transaction for you.

![txsent.png](./../../img/txsent.png)

### Common errors

- Make sure that the Ethereum Ledger app is installed. The Cosmos Ledger app is not supported on the Gridiron chain at the moment (see [FAQ](#faq)).
...
- If you have Metamask correctly configured, the balance on the extension will be displayed as `GRIDIRON`, but on the Ledger device it will be displayed as `ETH`.
...
Cosmos `secp256k1` keys are not supported on Gridiron with Ledger. Only Ethereum keys (`eth_secp256k1`) can be generated with Ledger.
...
Since Gridiron only support Ethereum keys and uses the same HD path as Ethereum, the Cosmos Ledger app doesn’t work to sign cosmos transactions.
