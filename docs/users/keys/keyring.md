...
```bash
gridirond keys add dev0

# Put the generated address in a variable for later use.
MY_VALIDATOR_ADDRESS=$(gridirond keys show dev0 -a)
```
...
::: warning
**NOTE**: Cosmos `secp256k1` keys are not supported on Gridiron due to compatibility issues with Ethereum transactions.
:::
...
yes $KEYPASSWD | gridirond keys add me
yes $KEYPASSWD | gridirond keys show me
# start gridirond with keyring-backend flag
gridirond --keyring-backend=file start
```
