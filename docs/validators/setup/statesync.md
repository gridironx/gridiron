...
```bash
SNAP_RPC1="http://bd-gridiron-testnet-state-sync-node-01.bdnodes.net:26657"
SNAP_RPC="http://bd-gridiron-testnet-state-sync-node-02.bdnodes.net:26657"
CHAIN_ID="gridiron_9000-4"
PEER="3a6b22e1569d9f85e9e97d1d204a1c457d860926@bd-gridiron-testnet-seed-node-01.bdnodes.net:26656"
wget -O $HOME/genesis.json https://archive.gridiron.dev/gridiron_9000-4/genesis.json 
```

## Use commands below for Mainnet setup

```bash
SNAP_RPC1="http://bd-gridiron-mainnet-state-sync-us-01.bdnodes.net:26657"
SNAP_RPC="http://bd-gridiron-mainnet-state-sync-eu-01.bdnodes.net:26657"
CHAIN_ID="gridiron_9001-2"
PEER="96557e26aabf3b23e8ff5282d03196892a7776fc@bd-gridiron-mainnet-state-sync-us-01.bdnodes.net,dec587d55ff38827ebc6312cedda6085c59683b6@bd-gridiron-mainnet-state-sync-eu-01.bdnodes.net"
wget -O $HOME/genesis.json https://archive.gridiron.org/mainnet/genesis.json 
```
...
```bash
gridirond init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .gridirond/config folder

```bash
mv $HOME/genesis.json ~/.gridirond/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.gridirond/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.gridirond/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.gridirond/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.gridirond/config/config.toml
```

### Create gridirond service

```bash
echo "[Unit]
Description=Gridirond Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which gridirond) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/gridirond.service; sudo mv $HOME/gridirond.service /etc/systemd/system/
```

```bash
sudo systemctl enable gridirond.service && sudo systemctl daemon-reload
```

### Run gridirond

```bash
sytemctl start gridirond
```

### Check logs

```bash
journalctl -u gridirond -f
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.gridirond/config/config.toml
```
