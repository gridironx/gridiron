...
Check the FAQ for running a validator on Gridiron {synopsis}
...
Gridiron is a public Proof-of-Stake (PoS) blockchain, meaning that validator's weight is determined by the amount of staking tokens (GRIDIRON) bonded as collateral. These staking tokens can be staked directly by the validator or delegated to them by GRIDIRON holders.
...
Delegators are GRIDIRON holders who cannot, or do not want to run validator operations themselves. Users can delegate GRIDIRON to a validator and obtain a part of its revenue in exchange (for more detail on how revenue is distributed, see [What is the incentive to stake?](#what-is-the-incentive-to-stake) and [What is a validator's commission?](#what-is-a-validators-commission) sections below).
...
- **Validator's Address**: `gridironvaloper1-` address. This is the address used to identify your validator publicly. The private key associated with this address is used to bond, unbond, and claim rewards.
...
- **Minimum self-bond amount**: Minimum amount of GRIDIRON the validator needs to have bonded at all times. If the validator's self-bonded stake falls below this limit, its entire staking pool will be unbonded.
- **Initial self-bond amount**: Initial amount of GRIDIRON the validator wants to self-bond.

```bash
gridirond tx staking create-validator
    --pubkey gridironvalconspub1zcjduepqs5s0vddx5m65h5ntjzwd0x8g3245rgrytpds4ds7vdtlwx06mcesmnkzly
    --amount "2agridiron"
    --from tmp
...
Once a validator is created and registered, GRIDIRON holders can delegate GRIDIRON to it, effectively adding stake to its pool. The total stake of a validator is the sum of the GRIDIRON self-bonded by the validator's operator and the GRIDIRON bonded by external delegators.
...
- **Tendermint Key**: This is a unique key used to sign block hashes. It is associated with a public key `gridironvalconspub`.
    - Generated when the node is created with `gridirond init`.
    - Get this value with `gridirond tendermint show-validator`
    e.g. `gridironvalconspub1zcjduc3qcyj09qc03elte23zwshdx92jm6ce88fgc90rtqhjx8v0608qh5ssp0w94c`

- **Application keys**: These keys are created from the application and used to sign transactions. As a validator, you will probably use one key to sign staking-related transactions, and another key to sign oracle-related transactions. Application keys are associated with a public key `gridironpub-` and an address `gridiron-`. Both are derived from account keys generated by `gridirond keys add`.

::: warning
A validator's operator key is directly tied to an application key, but uses reserved prefixes solely for this purpose: `gridironvaloper` and `gridironvaloperpub`
:::
...
- `unbonded`: Validator is not in the active set, and therefore not signing blocks. Unbonded validators cannot be slashed, but do not earn any rewards from their operation. It is still possible to delegate GRIDIRON to this validator. Un-delegating from an `unbonded` validator is immediate.

Delegators have the same state as their validator.

::: warning
Delegations are not necessarily bonded. GRIDIRON can be delegated and bonded, delegated and unbonding, delegated and unbonded, or liquid.
:::

### What is "self-bond"? How can I increase my "self-bond"?

The validator operator's "self-bond" refers to the amount of GRIDIRON stake delegated to itself. You can increase your self-bond by delegating more GRIDIRON to your validator account.

### Is there a testnet faucet?

<!-- markdown-link-check-disable-next-line -->
If you want to obtain coins for the testnet, you can do so by using the [faucet](https://faucet.gridiron.dev/).

### Is there a minimum amount of GRIDIRON that must be staked to be an active (bonded) validator?
...
### How often will a validator be chosen to propose the next block? Does it go up with the quantity of GRIDIRON staked?

The validator that is selected to mine the next block is called the **proposer**, the "leader" in the consensus for the round. Each proposer is selected deterministically, and the frequency of being chosen is equal to the relative total stake (where total stake = self-bonded stake + delegators stake) of the validator. For example, if the total bonded stake across all validators is 100 GRIDIRON, and a validator's total stake is 10 GRIDIRON, then this validator will be chosen 10% of the time as the proposer.
...
- **Block rewards:** Native tokens of applications run by validators (e.g. GRIDIRON on Gridiron) are inflated to produce block provisions. These provisions exist to incentivize GRIDIRON holders to bond their stake, as non-bonded GRIDIRON will be diluted over time.
- **Transaction fees:** Gridiron maintains a whitelist of token that are accepted as fee payment. The initial fee token is the `gridiron`.
...
Revenue received by a validator's pool is split between the validator and its delegators. The validator can apply a commission on the part of the revenue that goes to its delegators. This commission is set as a percentage. Each validator is free to set its initial commission, maximum daily commission change rate and maximum commission. Gridiron enforces the parameter that each validator sets. These parameters can only be defined when initially declaring candidacy, and may only be constrained further after being declared.

### How are block provisions distributed?

Block provisions (rewards) are distributed proportionally to all validators relative to their total stake (voting power). This means that even though each validator gains GRIDIRON with each provision, all validators will still maintain equal weight.

Let us take an example where we have 10 validators with equal staking power and a commission rate of 1%. Let us also assume that the provision for a block is 1000 GRIDIRON and that each validator has 20% of self-bonded GRIDIRON. These tokens do not go directly to the proposer. Instead, they are evenly spread among validators. So now each validator's pool has 100 GRIDIRON. These 100 GRIDIRON will be distributed according to each participant's stake:

- Commission: `100*80%*1% = 0.8 GRIDIRON`
- Validator gets: `100\*20% + Commission = 20.8 GRIDIRON`
- All delegators get: `100\*80% - Commission = 79.2 GRIDIRON`

Then, each delegator can claim its part of the 79.2 GRIDIRON in proportion to their stake in the validator's staking pool. Note that the validator's commission is not applied on block provisions. Note that block rewards (paid in GRIDIRON) are distributed according to the same mechanism.
...
Let's take a concrete example to illustrate the aforementioned concept. In this example, there are 10 validators with equal stake. Each of them applies a 1% commission and has 20% of self-bonded GRIDIRON. Now comes a successful block that collects a total of 1005 GRIDIRON in fees. Let's assume that the proposer included 100% of the signatures in its block. It thus obtains the full bonus of 5%.
...
- For the proposer validator:
    ...
    - The pool obtains $R ~ + ~ 5\%(R)$: 105 GRIDIRON
    - Commission: $105 ~ *~ 80\% ~* ~ 1\%$ = 0.84 GRIDIRON
    - Validator's reward: $105 ~ * ~ 20\% ~ + ~ Commission$ = 21.84 GRIDIRON
    - Delegators' rewards: $105 ~ * ~ 80\% ~ - ~ Commission$ = 83.16 GRIDIRON \(each delegator will be able to claim its portion of these rewards in proportion to their stake\)

    - The pool obtains $R$: 100 GRIDIRON
    - Commission: $100 ~ *~ 80\% ~* ~ 1\%$ = 0.8 GRIDIRON
    - Validator's reward: $100 ~ * ~ 20\% ~ + ~ Commission$ = 20.8 GRIDIRON
    - Delegators' rewards: $100 ~ * ~ 80\% ~ - ~ Commission$ = 79.2 GRIDIRON \(each delegator will be able to claim its portion of these rewards in proportion to their stake\)
...
### Do validators need to self-bond GRIDIRON
...
Even though there is no obligation for validators to self-bond GRIDIRON, delegators should want their validator to have self-bonded GRIDIRON in their staking pool. In other words, validators should have skin-in-the-game.

In order for delegators to have some guarantee about how much skin-in-the-game their validator has, the latter can signal a minimum amount of self-bonded GRIDIRON. If a validator's self-bond goes below the limit that it predefined, this validator and all of its delegators will unbond.

### How to prevent concentration of stake in the hands of a few top validators?

For now the community is expected to behave in a smart and self-preserving way. When a mining pool in Bitcoin gets too much mining power the community usually stops contributing to that pool. Gridiron will rely on the same effect initially. In the future, other mechanisms will be deployed to smoothen this process as much as possible:
...
In addition to running an Gridiron node, validators should develop monitoring, alerting and management solutions.

### What are bandwidth requirements?

Gridiron has the capacity for very high throughput compared to chains like Ethereum or Bitcoin.
...
The Gridiron team does not recommend one solution above the other. The community is encouraged to bolster the effort to improve HSMs and the security of key management.
