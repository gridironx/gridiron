...
Learn how to run an IBC Relayer for Gridiron. {synopsis}
...
## Prerequisites
<!-- textlint-disable -->
Before beginning, ensure you have an Gridiron node running in the background of the same machine that you intend to relay on. Follow [this guide](quickstart/run_node.md) to set up an Gridiron node if you have not already.
<!-- textlint-enable -->

In this guide, we will be relaying between [Gridiron (channel-3) and Cosmos Hub (channel-292)](https://www.mintscan.io/gridiron/relayers). When setting up your Gridiron and Cosmos full nodes, be sure to offset the ports being used in both the `app.toml` and `config.toml` files of the respective chains (this process will be shown below).

<!-- textlint-disable -->
In this example, the default ports for Gridiron will be used, and the ports of the Cosmos Hub node will be manually changed.
<!-- textlint-enable -->

## Gridiron Daemon Settings

First, set `grpc server` on port `9090` in the `app.toml` file from the `$HOME/.gridirond/config` directory:

```bash
vim $HOME/.gridirond/config/app.toml
```
...
Then, set the `pprof_laddr` to port `6060`, `rpc laddr` to port `26657`, and `prp laddr` to `26656` in the `config.toml` file from the `$HOME/.gridirond/config` directory:

```bash
vim $HOME/.gridirond/config/config.toml
```
...
```bash
# In this example, we will set channel-292 on the cosmoshub-4 chain settings and channel-3 on the gridiron_9001-2 chain settings:
[[chains]]
id = 'cosmoshub-4'
...
policy = 'allow'
list = [
   ['transfer', 'channel-292'], # gridiron_9001-2
]
...
hermes keys restore gridiron_9001-2 -m "24-word mnemonic seed"
```

Ensure this wallet has funds in both GRIDIRON and ATOM in order to pay the fees required to relay.
...
INFO ThreadId(01) [gridiron_9001-2] chain is healthy
```
...
```bash
hermes query packet unreceived-packets gridiron_9001-2 transfer channel-3
hermes query packet unreceived-acks gridiron_9001-2 transfer channel-3
```
...
hermes query packet commitments gridiron_9001-2 transfer channel-3
```
...
hermes clear packets gridiron_9001-2 transfer channel-3
```
...
```bash
hermes tx raw packet-recv gridiron_9001-2 cosmoshub-4 transfer channel-292
hermes tx raw packet-ack gridiron_9001-2 cosmoshub-4 transfer channel-292
hermes tx raw packet-recv cosmoshub-4 gridiron_9001-2 transfer channel-3
hermes tx raw packet-ack cosmoshub-4 gridiron_9001-2 transfer channel-3
```
