...
can be executed by passing the corresponding flags:
  ...
  - `--header`: Formats the markdown header 1 to fit the rest of the Gridiron and Ethermint docs
...
def format_header_in_file(file: str) -> None:
    ...
    match the Gridiron and Ethermint docs.
...
def add_metadata(file: str, position: int, title: str, parent: str) -> None:
    ...
    (which are used for the 'Modules' dropdown on the Gridiron docs).
