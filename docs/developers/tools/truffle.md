...
Learn how to deploy a simple Solidity-based smart contract to Gridiron using the Truffle environment {synopsis}
...
This will allow your contract to connect to your Gridiron local node.
...
You should see incoming deployment logs in the Gridiron daemon Terminal tab for each transaction (one to deploy `Migrations.sol` and the other to deploy `Counter.sol`).
...
Now, you can run the Truffle tests using the Gridiron node using the `test` command:
