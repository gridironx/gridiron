...
Learn how to run and setup the JSON-RPC server on Gridiron. {synopsis}

:::tip
**Important**: You cannot use all JSON RPC methods unless your node stores the entire copy of the blockchain locally. Do you need archives/snapshots of our networks? Go to [this section](https://docs.gridiron.org/validators/snapshots_archives.html).
:::

## Introduction

JSON-RPC is provided on multiple transports. Gridiron supports JSON-RPC over HTTP and WebSocket.
...
```bash
gridirond start --json-rpc.enable
```
...
```bash
gridirond start --json-rpc.enable-indexer 
```
...
```bash
gridirond index-eth-tx backward
```
...
```bash
gridirond index-eth-tx forward
```
