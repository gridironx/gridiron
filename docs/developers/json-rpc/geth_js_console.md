...
Now we can use all implemented JSON-RPC methods. Find an exhaustive list of the supported JSON-RPC methods on [Gridiron docs](https://docs.gridiron.org/developers/json-rpc/endpoints.html).
...
We get a big number because the result is denominated in `agridiron`. We can convert to Gridiron (10 ^18 `agridiron`) using the `web3.fromWei` method:
...
We can perform token transfers using the corresponding method. For example, let's transfer 1 Gridiron token from our account to another account:
...
The sender balance is reduced by 1 Gridiron token and the fees paid for the transaction:
...
The receiver account balance initially was 100000000 Gridiron tokens. After the transaction, the account balance has increased by 1 Gridiron token.
