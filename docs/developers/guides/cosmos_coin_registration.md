...
Learn how to register native Cosmos Coins through [Gridiron Governance](../../users/governance/overview.md). {synopsis}
...
To [submit the proposal](../../users/governance/submitting.md) to testnet through the command line with [`gridirond`](../../validators/quickstart/binary.md), use the following command with `register-coin`:

```bash
gridirond tx gov submit-legacy-proposal register-coin <path/to/metadata.json> \
  --title=<title> \
...
```bash
gridirond tx gov submit-legacy-proposal register-coin --proposal=<path/to/proposal.json>
```
