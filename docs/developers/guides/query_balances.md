...
Learn how to query balances of IBC Cosmos Coins and ERC-20s on Gridiron. {synopsis}

This guide will cover the following query methods:

- [`gridirond` & Tendermint RPC](#gridirond--tendermint-rpc)
- [JSON-RPC](#json-rpc)
- [gRPC](#grpc)

:::warning
**Note**: In this document, the command line is used to interact with endpoints. For dApp developers, using libraries such as [cosmjs](https://github.com/cosmos/cosmjs) and [gridironjs](../libraries/gridironjs.md) is recommended instead.
:::

## `gridirond` & Tendermint RPC

Upon [installation](../../validators/quickstart/installation.md) and [configuration](../../validators/quickstart/binary.md) of the Gridiron Daemon, developers can query account balances using `gridirond` with the following CLI command:

```bash
$ gridirond query bank balances $GRIDIRONADDRESS --count-total=$COUNTTOTAL --height=$HEIGHT --output=$OUTPUT --node=$NODE
balances:
...
- `$GRIDIRONADDRESS` is the Gridiron address with balances of interest (eg. `gridiron1...`).
...
- (optional if running local node) `$NODE` is the Tendermint RPC node information is requested from (eg. `https://tendermint.bd.gridiron.org:26657`).

Details of non-native currencies (ie. not `agridiron`) can be queried with the following CLI command:

```bash
$ gridirond query erc20 token-pair $DENOM --node=$NODE --height=$HEIGHT --output=$OUTPUT
token_pair:
...
Developers can query account balances of `agridiron` using the [`eth_getBalance`](../json-rpc/endpoints.md#ethgetbalance) JSON-RPC method in conjunction with [`curl`](https://curl.se/):
...
- `$ETHADDRESS` is the Etherum hex-address the balance is to be queried from.
    Note that Gridiron addresses (those beginning with `gridiron1...`) can be converte.d to Ethereum addresses using libraries such as [gridironjs](../libraries/gridironjs.md).
...
- (optional if running local node) `$NODE` is the JSON-RPC node information is requested from (eg. `https://eth.bd.gridiron.org:8545`).
...
```bash
# Request
grpcurl $OUTPUT -d '{"address":`$GRIDIRONADDRESS`}' $NODE cosmos.bank.v1beta1.Query/AllBalances
...
- `$GRIDIRONADDRESS` is the Gridiron address with balances of interest (eg. `"gridiron1..."`).
- `$NODE` is the Cosmos gRPC node information is requested from (eg. `https://grpc.bd.gridiron.org:9090`).
...
func queryState() error {
    myAddress, err := GetGridironAddressFromBech32("gridiron1...") // gridiron address with balances of interest.
    if err != nil {
        return err
    }
    ...
    grpcConn := grpc.Dial(
        "https://grpc.bd.gridiron.org:9090", // your gRPC server address.
...
// gridironjs address converter.
func GetGridironAddressFromBech32(address string) (string, error) {...}
```

:::tip
**Note**: The following tools will be useful when using gRPC:

- [Gridiron Swagger API](https://api.gridiron.dev/): a comprehensive description of all gRPC endpoints
- [Cosmos SDK Go API](https://pkg.go.dev/github.com/cosmos/cosmos-sdk) & [Gridiron Go API](https://pkg.go.dev/github.com/tharsis/gridiron): packages to implement queries in Go scripts
