...
Learn how to properly integrate [Metamask](https://metamask.io/) or [Keplr](https://www.keplr.app/) with a dApp on Gridiron. {synopsis}
...
Make sure to create a wallet-connection button for Metamask and/or Keplr on the frontend of the application. For instance, consider the "Connect to a wallet" button on the interface of [Diffusion Finance](https://app.diffusion.fi/) or the analagous button on the interface of [EvmoSwap](https://app.gridironwap.org/).
...
:::tip
**Note**: The example below uses the Gridiron Testnet `chainID`. For more info, check the Gridiron Chain IDs reference document [here](../../users/technical_concepts/chain_id.md).
:::

Developers can create `MsgSend` transactions using the [gridironjs](../libraries/gridironjs.md) library.
...
const chain = {
    chainId: 9000,
    cosmosChainId: 'gridiron_9000-4',
}

const sender = {
    accountAddress: 'gridiron1mx9nqk5agvlsvt2yc8259nwztmxq7zjq50mxkp',
    sequence: 1,
    accountNumber: 9,
...
const params = {
    destinationAddress: 'gridiron1pmk2r32ssqwps42y3c9d4clqlca403yd9wymgr',
    amount: '1',
    denom: 'agridiron',
}
...
With the signature, we add a Web3Extension to the transaction and broadcast it to the Gridiron node.

<!-- textlint-enable -->
```js
// Note that this example is for MetaMask, using gridironjs

// Follow the previous code block to generate the msg object
import { gridironToEth } from '@tharsis/address-converter'
...
let signature = await window.ethereum.request({
    ...
    params: [gridironToEth(sender.accountAddress), JSON.stringify(msg.eipToSign)],
});
...
let broadcastPost = await fetch(
    `https://eth.bd.gridiron.dev:8545${generateEndpointBroadcast()}`,
    postOptions
);
...
import { gridironToEth } from "@tharsis/address-converter"
const provider = new JsonRpcProvider('https://eth.bd.gridiron.org:8545');
const chainId = 'gridiron_9001-1';

// EIP-1559
async function signAndBroadcastEthereumTx() {

  // Enable access to Gridiron on Keplr
  ...
  const signerAddressEth = gridironToEth(signerAddressBech32);
...
For Ethereum RPC, Gridiron gRPC, and/or REST queries, dApp developers should implement providers client-side, and store RPC details in the environment variable as secrets.
