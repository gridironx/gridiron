...
This command creates a 4-node network using the `gridirondnode` Docker image.
The ports for each node are found in this table:

| Node ID          | P2P Port | Tendermint RPC Port | REST/ Ethereum JSON-RPC Port | WebSocket Port |
|------------------|----------|---------------------|------------------------------|----------------|
| `gridironnode0` | `26656`  | `26657`             | `8545`                       | `8546`         |
| `gridironnode1` | `26659`  | `26660`             | `8547`                       | `8548`         |
| `gridironnode2` | `26661`  | `26662`             | `8549`                       | `8550`         |
| `gridironnode3` | `26663`  | `26664`             | `8551`                       | `8552`         |
...
```bash
...
Creating network "gridiron_localnet" with driver "bridge"
Creating gridirondnode0 ... done
Creating gridirondnode2 ... done
Creating gridirondnode1 ... done
Creating gridirondnode3 ... done
```
...
calling the `gridirond testnet` command. This outputs a handful of files in the
`./build` directory:
...
Each `./build/nodeN` directory is mounted to the `/gridirond` directory in each container.
...
```bash
# node 0: daemon logs
docker exec gridirondnode0 tail gridirond.log
...
To interact with `gridirond` and start querying state or creating txs, you use the
`gridirond` directory of any given node as your `home`, for example:

```bash
gridirond keys list --home ./build/node0/gridirond
```
...
::: tip
**Note**: Each node's seed is located at `./build/nodeN/gridirond/key_seed.json` and can be restored to the CLI using the `gridirond keys add --restore` command
:::
...
BINARY=gridiron make localnet-start
```
